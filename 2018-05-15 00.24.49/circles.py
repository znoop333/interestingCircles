...
class Circle: 
    ...
    def draw(self): 
        ...
        self.history.append(((self.x - self.radius, self.y - self.radius, self.x + self.radius, self.y + self.radius), BG_COLOR))#choice(R_N_B)))
    ...
    def step(self):
        ...
        if self.x  >= IMAGE_WIDTH + self.radius*100 or self.x <= 0 - self.radius*100:
            self.active = False
            self.oob_deactiavte = True
        #if self.y + self.radius >= IMAGE_HEIGHT  or self.y - self.radius <= 0:
        if self.y  >= IMAGE_HEIGHT + self.radius*100 or self.y <= 0 - self.radius*100:
            self.active = False
            self.oob_deactiavte = True
        if self.radius <= 0:
            self.active = False
...
CURRENT_IMAGE = CURRENT_IMAGE.filter(ImageFilter.GaussianBlur(.5))
CURRENT_IMAGE.save(os.path.join(SAVE_DIR, "{}.png".format("blurred")))
