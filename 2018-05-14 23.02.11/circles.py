...
class Circle: 
    ...
    def draw(self): 
        ...
        self.history.append(((self.x - self.radius, self.y - self.radius, self.x + self.radius, self.y + self.radius), choice(EARTH_WORM)))
...
for _ in range(5000):
    rad = randint(10, 30)
    x = randint(IMAGE_WIDTH*.2, IMAGE_WIDTH*.8)
    y = randint(IMAGE_HEIGHT*.2, IMAGE_HEIGHT*.8)
