...
class Circle: 
    ...
    def draw(self): 
        ...
        self.history.append(((self.x - self.radius, self.y - self.radius, self.x + self.radius, self.y + self.radius), choice(BLUES + RED_ORANGES + [random_grey() for _ in range(30)])))
...
for _ in range(5000):
    rad = randint(10, 30)
    x = IMAGE_WIDTH/2 #randint(IMAGE_WIDTH*.2, IMAGE_WIDTH*.8)
    y = IMAGE_HEIGHT/2
